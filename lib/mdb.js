...
var validateSDBRequest = function(SDB, secretKey) {
  ...
  var nvps = SDB.nvps;
  var type;
  var stringToSign = createStringToSign(SDB);
  if (stringToSign === -1) {
...
var Select = function(SDB) {
  ...
  //console.log("Select Expression = " + expression);
...
var select = function(domainIndex, SDB) {
  ...
  if ((SDB.select.items === '*')&&(SDB.select.condition === '')) {
    SDB.select.xml = {};
    db.clientPool[db.connection()].getAllSubscripts('MDB', [accessKeyId, 'domains', domainIndex, 'items'], function (error, results) {
      if (trace) console.log("itemIndex array = " + JSON.stringify(results));
      SDB.select.items = results;
      if (results.length > 0) {
        // now get attribs Names
        db.clientPool[db.connection()].getJSON('MDB', [accessKeyId, 'domains', domainIndex, 'attribs'], function (error, results) {
...
var getItemDetails = function(itemIndex, SDB) {
  ...
  //console.log("get details for itemIndex = " + itemIndex);
  db.clientPool[db.connection()].getJSON('MDB', [accessKeyId, 'domains', domainIndex, 'items', itemIndex], function (error, results) {
    //console.log("item contents = " + JSON.stringify(results));
...
var addDomain = function(SDB) {
  ...
  var domainName = nvps.DomainName;
  var count = 0;
  db.clientPool[db.connection()].increment('MDB', [accessKeyId], 1, function (error, results) {
    count++;
    sendCreateDomainResponse(count, SDB);
  });
  db.clientPool[db.connection()].increment('MDB', [accessKeyId, 'domains'], 1, function (error, results) {
    var id = results.value;
    var now = new Date().getTime();
    var nameIndex = domainName.substring(0,MDB.indexLength);
    db.clientPool[db.connection()].setGlobal('MDB', [accessKeyId, 'domains', id, 'name'], domainName, function (error, results) {
      count++;
      sendCreateDomainResponse(count, SDB); 
    });
    db.clientPool[db.connection()].setGlobal('MDB', [accessKeyId, 'domains', id, 'created'], now, function (error, results) {
      count++;
      sendCreateDomainResponse(count, SDB);
    });
    db.clientPool[db.connection()].setGlobal('MDB', [accessKeyId, 'domains', id, 'modified'], now, function (error, results) {
      count++;
      sendCreateDomainResponse(count, SDB);
    });
    db.clientPool[db.connection()].setGlobal('MDB', [accessKeyId, 'domainIndex', nameIndex, id], '', function (error, results) {
      count++;
      sendCreateDomainResponse(count, SDB);
    });
  });
...
var removeDomain = function(index, SDB) {
  ...
  var count = 0;
  var nameIndex = domainName.substring(0,MDB.indexLength);
  db.clientPool[db.connection()].kill('MDB', [accessKeyId, 'domains', index], function (error, results) {
    count++;
    sendDeleteDomainResponse(count, SDB);
  });
  db.clientPool[db.connection()].kill('MDB', [accessKeyId, 'domainIndex', nameIndex, index], function (error, results) {
    count++;
    sendDeleteDomainResponse(count, SDB);
  });
  db.clientPool[db.connection()].decrement('MDB', [accessKeyId], 1, function (error, results) {
    count++;
    sendDeleteDomainResponse(count, SDB);
  });
};
...
var getMetadata = function(index, SDB) {
  ...
  db.clientPool[db.connection()].getGlobal('MDB', [accessKeyId, 'domains', index, 'modified'], function (error, results) {
  ...
  db.clientPool[db.connection()].getAllSubscripts('MDB', [accessKeyId, 'domains', index, 'items'], function (error, results) {
    ...
    for (var i = 0; i < noOfItems; i++) {
      itemId = results[i];
      db.clientPool[db.connection()].getGlobal('MDB', [accessKeyId, 'domains', index, 'items', itemId], function (error, results) {
      ...
      db.clientPool[db.connection()].getJSON('MDB', [accessKeyId, 'domains', index, 'items', itemId], function (error, results) {
        //console.log("attributes : " + JSON.stringify(results));
        attributesChecked++;
        var attribs = results.attribs;
  ...
  db.clientPool[db.connection()].getGlobal('MDB', [accessKeyId, 'domains', index, 'attribs'], function (error, results) {
    var noOfAttribs = parseInt(results.value);
    var attribName;
    metadata.AttributeNameCount = 0;
    metadata.AttributeNamesSizeBytes = 0;
    db.clientPool[db.connection()].getJSON('MDB', [accessKeyId, 'domains', index, 'attribs'], function (error, results) {
      var attribNo;
...
var getAttributes = function(domainIndex, SDB) {
  ...
  if (itemName) {
    ...
    db.clientPool[db.connection()].getJSON('MDB', [accessKeyId, 'domains', domainIndex, 'itemIndex', nameIndex], function (error, results) {
      ...
      if (count === 0) {
        ...
        var noChecked = 0;
        for (index in results) {
          db.clientPool[db.connection()].getGlobal('MDB', [accessKeyId, 'domains', domainIndex, 'items', index],
            function (error, results) {
              ...
              if (noChecked === count) {
                ...
                if (!matchFound) {
                  ...
                  db.clientPool[db.connection()].getJSON('MDB', [accessKeyId, 'domains', domainIndex, 'attribs'], function (error, results) {
                    ...
                    db.clientPool[db.connection()].getJSON('MDB', [accessKeyId, 'domains', domainIndex, 'items', index, 'attribs'], function (error, results) {
                      var attribNo;
                      var attribName;
                      var values;
                      var valueNo;
...
var addItemToDomain = function(domainIndex, itemName, SDB) {
  ...
  if (SDB.expected.name) {
    ...
    var nvps = SDB.nvps;
    var accessKeyId = nvps.AWSAccessKeyId;
    db.clientPool[db.connection()].increment('MDB', [accessKeyId, 'domains', domainIndex, 'items'], 1, function (error, results) {
      var itemIndex = results.value;
      var itemNameIndex = itemName.substring(0,MDB.indexLength);
      var count = 0;
      var done = 7;
      db.clientPool[db.connection()].setGlobal('MDB', [accessKeyId, 'domains', domainIndex, 'itemIndex', itemNameIndex, itemIndex], '', function (error, results) { 
        count++;
        if (count === done) addAttributes(domainIndex, itemIndex, SDB);
      });
      db.clientPool[db.connection()].setGlobal('MDB', [accessKeyId, 'domains', domainIndex, 'items', itemIndex], itemName, function (error, results) { 
        count++;
        if (count === done) addAttributes(domainIndex, itemIndex, SDB);
      });
      db.clientPool[db.connection()].setGlobal('MDB', [accessKeyId, 'domains', domainIndex, 'attribs', '0'], 'itemName()', function (error, results) { 
        count++;
        if (count === done) addAttributes(domainIndex, itemIndex, SDB);
      });
      db.clientPool[db.connection()].setGlobal('MDB', [accessKeyId, 'domains', domainIndex, 'attribsIndex', 'itemName()', 0], '', function (error, results) { 
        count++;
        if (count === done) addAttributes(domainIndex, itemIndex, SDB);
      });
      db.clientPool[db.connection()].setGlobal('MDB', [accessKeyId, 'domains', domainIndex, 'items', itemIndex, 'attribs', 0, 'value'], '1', function (error, results) { 
        count++;
        if (count === done) addAttributes(domainIndex, itemIndex, SDB);
      });
      db.clientPool[db.connection()].setGlobal('MDB', [accessKeyId, 'domains', domainIndex, 'items', itemIndex, 'attribs', 0, 'value', 1], itemName, function (error, results) { 
        count++;
        if (count === done) addAttributes(domainIndex, itemIndex, SDB);
      });
      db.clientPool[db.connection()].setGlobal('MDB', [accessKeyId, 'domains', domainIndex, 'queryIndex', '0', itemNameIndex, itemIndex], '', function (error, results) { 
        count++;
        if (count === done) addAttributes(domainIndex, itemIndex, SDB);
      });
    });
...
var checkExpectedValue = function(domainIndex, itemIndex, attrIndex, SDB) {
  var attributes = SDB.attributes;
  db.clientPool[db.connection()].getGlobal('MDB', [SDB.nvps.AWSAccessKeyId, 'domains', domainIndex, 'items', itemIndex, 'attribs', attrIndex, 'value', 1], function (error, results) {
...
var addAttributeIndexToDomain = function(domainIndex, itemIndex, attrName, SDB) {
  ...
  var nvps = SDB.nvps;
  var accessKeyId = nvps.AWSAccessKeyId;
  db.clientPool[db.connection()].increment('MDB', [accessKeyId, 'domains', domainIndex, 'attribs'], 1, function (error, results) {
    var attrIndex = results.value;
    var attrNameIndex = attrName.substring(0,MDB.indexLength);
    var count = 0;
    db.clientPool[db.connection()].setGlobal('MDB', [accessKeyId, 'domains', domainIndex, 'attribs', attrIndex], attrName, function (error, results) { 
      count++;
      if (count === 2) addAttribute(domainIndex, itemIndex, attrIndex, attrName, SDB);
    });
    db.clientPool[db.connection()].setGlobal('MDB', [accessKeyId, 'domains', domainIndex, 'attribsIndex', attrNameIndex, attrIndex], '', function (error, results) { 
      count++;
      if (count === 2) addAttribute(domainIndex, itemIndex, attrIndex, attrName, SDB);
    });
  });
...
var removeOldValues = function(domainIndex, itemIndex, attrIndex, values, SDB) {
    var accessKeyId = SDB.nvps.AWSAccessKeyId;
      db.clientPool[db.connection()].getJSON('MDB', [accessKeyId, 'domains', domainIndex, 'items', itemIndex, "attribs", attrIndex, 'value'], function (error, results) {
        ...
        for (valueIndex in results) {
          ...
          db.clientPool[db.connection()].kill('MDB', [accessKeyId, 'domains', domainIndex, 'items', itemIndex, "attribs", attrIndex, 'valueIndex', value, valueIndex], function (error, results) {
            count++;
            if (trace) console.log("1 count = " + count);
            if (count === max) addNewAttrValues(domainIndex, itemIndex, attrIndex, values, SDB);
          });
          db.clientPool[db.connection()].kill('MDB', [accessKeyId, 'domains', domainIndex, 'queryIndex', attrIndex, valueNameIndex, itemIndex], function (error, results) {
            count++;
            if (trace) console.log("2 count = " + count);
            if (count === max) addNewAttrValues(domainIndex, itemIndex, attrIndex, values, SDB);
          });
        }
        db.clientPool[db.connection()].kill('MDB', [accessKeyId, 'domains', domainIndex, 'items', itemIndex, 'attribs', attrIndex, 'value'], function (error, results) {
          count++;
...
var addNewAttrValue = function(max, domainIndex, itemIndex, attrIndex, value, SDB) {
  ...
  db.clientPool[db.connection()].getJSON('MDB', [accessKeyId, 'domains', domainIndex, 'items', itemIndex, "attribs", attrIndex], function (error, results) {
    ...
    if (exists) {
      ...
      if (SDB.count === SDB.max) sendPutAttributesResponse(SDB);
    }
    else {
      db.clientPool[db.connection()].increment('MDB', [accessKeyId, 'domains', domainIndex, 'items', itemIndex, 'attribs', attrIndex, 'value'], 1, function (error, results) {
        var valueIndex = results.value;
        db.clientPool[db.connection()].setGlobal('MDB', [accessKeyId, 'domains', domainIndex, 'items', itemIndex, 'attribs', attrIndex, 'value', valueIndex], value, function (error, results) { 
          SDB.count++;
          if (trace) console.log("1: " + SDB.count + "; " + SDB.max + "; valueIndex=" + valueIndex);
          if (SDB.count === SDB.max) sendPutAttributesResponse(SDB);
        });
        db.clientPool[db.connection()].setGlobal('MDB', [accessKeyId, 'domains', domainIndex, 'items', itemIndex, 'attribs', attrIndex, 'valueIndex', valueNameIndex, valueIndex], '', function (error, results) { 
          SDB.count++;
        ...
        db.clientPool[db.connection()].setGlobal('MDB', [accessKeyId, 'domains', domainIndex, 'queryIndex', attrIndex, valueNameIndex, itemIndex], '', function (error, results) { 
          SDB.count++;
...
var sendPutAttributesResponse = function(SDB) {
  okResponse(SDB);
};
...
var removeItem = function(domainIndex, itemIndex, SDB) {
  ...
  var itemNameIndex = itemName.substring(0,MDB.indexLength);
  var accessKeyId = nvps.AWSAccessKeyId;
  db.clientPool[db.connection()].getJSON('MDB', [accessKeyId, 'domains', domainIndex, 'items', itemIndex, "attribs"], function (error, results) {
    ...
    for (attribId in results) {
      ...
      if (typeof attribObj.valueIndex !== 'undefined') {
        for (value in attribObj.valueIndex) {
          db.clientPool[db.connection()].kill('MDB', [accessKeyId, 'domains', domainIndex, 'queryIndex', attribId, value, itemIndex], function (error, results) {
            count++;
            if (trace) console.log("deleted record: count=" + count);
            if (count === max) {
              db.clientPool[db.connection()].kill('MDB', [accessKeyId, 'domains', domainIndex, 'items', itemIndex], function (error, results) {
                killCount++;
                if (killCount === 3) okResponse(SDB);
              });
              db.clientPool[db.connection()].kill('MDB', [accessKeyId, 'domains', domainIndex, 'itemIndex', itemNameIndex, itemIndex], function (error, results) {
                killCount++;
                if (killCount === 3) okResponse(SDB);
              });
              db.clientPool[db.connection()].kill('MDB', [accessKeyId, 'domains', domainIndex, 'queryIndex', '0', itemNameIndex], function (error, results) {
                killCount++;
                if (killCount === 3) okResponse(SDB);
              });
            }
...
var removeAttribute = function(domainIndex, itemIndex, attrIndex, attrName, SDB) {
  ...
  var valueIndex;
  db.clientPool[db.connection()].getJSON('MDB', [accessKeyId, 'domains', domainIndex, 'items', itemIndex, "attribs", attrIndex], function (error, results) {
    if (trace) console.log("attr records: " + JSON.stringify(results));
    if (typeof results.valueIndex !== 'undefined') {
      var noOfValues = 0;
...
var removeValueFromAttr = function(domainIndex, itemIndex, attrIndex, attrName, valueIndex, valueNameIndex, SDB, counter) {
  ...
  db.clientPool[db.connection()].kill('MDB', [accessKeyId, 'domains', domainIndex, 'items', itemIndex, 'attribs', attrIndex, 'value', valueIndex], function (error, results) {
    valRecCount++;
    if (valRecCount == 3) {
      counter.count++;
      if (counter.count === counter.noOfValues) removeAttrFromItem(domainIndex, itemIndex, attrIndex, attrName, SDB);
  ...
  db.clientPool[db.connection()].kill('MDB', [accessKeyId, 'domains', domainIndex, 'items', itemIndex, 'attribs', attrIndex, 'valueIndex', valueNameIndex, valueIndex], function (error, results) {
    valRecCount++;
    if (valRecCount == 3) {
      counter.count++;
      if (counter.count === counter.noOfValues) removeAttrFromItem(domainIndex, itemIndex, attrIndex, attrName, SDB);
...
var removeAttrFromItem = function(domainIndex, itemIndex, attrIndex, attrName, SDB) {
  ...
  //db.clientPool[db.connection()].kill('MDB', [accessKeyId, 'domains', domainIndex, 'items', itemIndex, 'attribs', attrIndex], function (error, results) {
    ...
    db.clientPool[db.connection()].getJSON('MDB', [accessKeyId, 'domains', domainIndex, 'queryIndex', attrIndex], function (error, results) {
      ...
      if (!attrStillReferenced) {
        ...
        var count = 0;
        var attrNameIndex = attrName.substring(0,MDB.indexLength);
        db.clientPool[db.connection()].kill('MDB', [accessKeyId, 'domains', domainIndex, 'attribs', attrIndex], function (error, results) {
          count++;
          if (count == 2) removeItemFromDomain(domainIndex, itemIndex, SDB);
        });
        db.clientPool[db.connection()].kill('MDB', [accessKeyId, 'domains', domainIndex, 'attribsIndex', attrNameIndex], function (error, results) {
          count++;
          if (count == 2) removeItemFromDomain(domainIndex, itemIndex, SDB);
        });
      }
...
var removeItemFromDomain = function(domainIndex, itemIndex, SDB) {
  ...
  db.clientPool[db.connection()].getJSON('MDB', [accessKeyId, 'domains', domainIndex, 'items', itemIndex, 'attribs'], function (error, results) {
    ...
    for (itemNo in results) {
      ...
      if (typeof results[itemNo].valueIndex !== 'undefined') itemStillReferenced = true;
      break;
    }
...
var sendDeleteAttributesResponse = function(SDB) {
  ...
  if (SDB.NoOfAttributesDeleted === SDB.NoOfAttributesToDelete) okResponse(SDB);
}
...
var checkDomainExists = function(invoke, SDB) {
  ...
  if (domainName) {
    ...
    var index;
    var nameIndex = domainName.substring(0,MDB.indexLength);
    db.clientPool[db.connection()].getJSON('MDB', [accessKeyId, 'domainIndex', nameIndex], function (error, results) {
      ...
      if (count === 0) {
        ...
        //console.log("domainName may exist: checking..");
        var noChecked = 0;
        for (index in results) {
          db.clientPool[db.connection()].getGlobal('MDB', [accessKeyId, 'domains', index, 'name'],
            function (error, results) {
...
var checkAttributeExists = function(domainIndex, itemIndex, attributeName, inItem, invoke, SDB) {
  ...
  db.clientPool[db.connection()].getJSON('MDB', [accessKeyId, 'domains', domainIndex, 'attribsIndex', nameIndex], function (error, results) {
...
var checkAttrIndex = function(inItem, domainIndex, itemIndex, attributeName, attrIndex, invoke, SDB) {
  var accessKeyId = SDB.nvps.AWSAccessKeyId;
  db.clientPool[db.connection()].getGlobal('MDB', [accessKeyId, 'domains', domainIndex, 'attribs', attrIndex],function (error, results) {
    ...
    if (SDB.checkAttr[attributeName].noOfAttrsChecked === SDB.checkAttr[attributeName].noAttrsToCheck) {
      ...
      if (SDB.checkAttr[attributeName].matchingAttrFound) {
        if (inItem) {
          ...
          db.clientPool[db.connection()].getGlobal('MDB', [accessKeyId, 'domains', domainIndex, 'items', itemIndex, "attribs", SDB.checkAttr[attributeName].attrIndex],function (error, results) {
...
var createStringToSign = function(SDB) {
  var stringToSign;
    ...
    case '2':
      ...
      */
      ...
      for (var i=0; i < keys.length; i++) {
        ...
        stringToSign = stringToSign + amp + sdbEscape(name) + '=' + sdbEscape(value);
        amp = '&';
      }
      break;
    default:
