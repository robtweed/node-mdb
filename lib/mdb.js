...
var validateSDBRequest = function(SDB, secretKey) {
  ...
  var nvps = SDB.nvps;
  var type;
  var stringToSign = createStringToSign(SDB, true);
  if (stringToSign === -1) {
    ...
    if (hash === nvps.Signature) {
      ...
      stringToSign = createStringToSign(SDB, false);
...
var Select = function(SDB) {
  ...
  console.log("*** SDB.select = " + JSON.stringify(SDB.select));
...
var createStringToSign = function(SDB, includePort) {
  var stringToSign;
